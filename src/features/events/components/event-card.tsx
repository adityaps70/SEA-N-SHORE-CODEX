    ...
    <article className="overflow-hidden rounded-[1.5rem] border border-mist-100 bg-white shadow-[var(--shadow-card)] transition hover:-translate-y-0.5 hover:shadow-lg">
      ...
      <div className="space-y-4 p-5">
        ...
        <div className="grid gap-2 text-sm text-navy-700">
          <span className="flex items-center gap-2"><CalendarDays className="h-4 w-4 text-teal-700" />{dateLabel(event.startAt)}</span>
          <span className="flex items-center gap-2">{event.format === 'online' ? <Monitor className="h-4 w-4 text-teal-700" /> : <MapPin className="h-4 w-4 text-teal-700" />}{event.format === 'online' ? 'Online' : event.locationName ?? 'Venue to be confirmed'}</span>
