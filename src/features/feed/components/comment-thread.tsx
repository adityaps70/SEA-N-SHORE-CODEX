  ...
  return (
    <form ref={formRef} action={formAction} className="mt-2 flex items-end gap-2">
      ...
      <div className="min-w-0 flex-1">
        ...
        <textarea ref={inputRef} id={`reply-${parentCommentId}`} name="body" rows={1} defaultValue={state.value ?? ''} placeholder="Write a reply…" className="min-h-10 w-full resize-y rounded-xl border border-mist-100 bg-white px-3 py-2 text-sm text-ink placeholder:text-muted" />
...
export function CommentThread({
  ...
  useEffect(() => {
    if (composerOpen && !readOnly) inputRef.current?.focus()
      ...
      {composerOpen && !readOnly ? (
        <form ref={formRef} action={formAction} className="mt-3 flex items-end gap-2">
          ...
          <div className="min-w-0 flex-1">
            ...
            <textarea ref={inputRef} id={`comment-${postId}`} name="body" rows={1} defaultValue={state.value ?? ''} placeholder="Add a professional comment…" className="min-h-11 w-full resize-y rounded-xl border border-mist-100 bg-white px-3 py-2.5 text-sm text-ink placeholder:text-muted" />
            {state.fieldErrors?.body ? <p className="mt-1 text-xs text-red-700">{state.fieldErrors.body[0]}</p> : null}
          </div>
