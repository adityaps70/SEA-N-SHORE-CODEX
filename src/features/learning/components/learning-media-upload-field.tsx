...
export function LearningMediaUploadField({
  ...
  }, [courseId, kind, pendingPath, value])
  ...
  return (
    <div className="rounded-2xl border border-mist-200 bg-mist-50/40 p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs leading-5 text-muted">
            {previewType === 'image' ? 'JPEG, PNG or WebP · up to 5 MB' : kind === 'lesson_video' ? 'MP4 or WebM · up to 500 MB' : 'MP4 or WebM · up to 200 MB'}
          </p>
        </div>
        {value ? (
          <button
      ...
      >
        {uploading ? <Loader2 className="size-6 animate-spin text-teal-700" aria-hidden="true" /> : previewType === 'image' ? <ImageIcon className="size-6 text-teal-700" aria-hidden="true" /> : value ? <PlayCircle className="size-6 text-teal-700" aria-hidden="true" /> : <UploadCloud className="size-6 text-teal-700" aria-hidden="true" />}
      ...
      {progress !== null ? (
        ...
        <p className="mt-3 text-xs text-muted">Media attached. Save your course or lesson to persist this change.</p>
      ) : null}
