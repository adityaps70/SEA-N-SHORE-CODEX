...
type ProgressQuery = (text: string, values?: readonly unknown[]) => Promise<QueryResultRow[]>
type ProgressTransaction = <T>(work: (query: ProgressQuery) => Promise<T>) => Promise<T>
...
function runtimeTransaction<T>(work: (query: ProgressQuery) => Promise<T>) {
