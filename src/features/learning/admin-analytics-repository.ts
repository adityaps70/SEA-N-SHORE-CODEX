...
export type AdminLearningAnalyticsQuery = (text: string, values?: readonly unknown[]) => Promise<QueryResultRow[]>

type AdminAuthorizationRow = QueryResultRow & { allowed?: boolean }
type ActiveMentorCountRow = QueryResultRow & { active_mentor_count: string | number | null }
type AdminCourseAnalyticsRow = QueryResultRow & {
  course_id: string
  slug: string
  title: string
  status: string
...
function mapCourse(row: AdminCourseAnalyticsRow): AdminCourseAnalytics {
...
export function createAdminLearningAnalyticsRepository(input: { query?: AdminLearningAnalyticsQuery } = {}) {
  const queryRows: AdminLearningAnalyticsQuery = input.query ?? ((text, values) => databaseQuery<QueryResultRow>(text, values))
  ...
  async function requirePlatformAdministrator(userId: string) {
    ...
    const row = rows[0]
    if (!row || (row.allowed !== undefined && !Boolean(row.allowed))) throw new Error('admin_forbidden')
  }

  async function getPlatformAnalytics(adminUserId: string): Promise<AdminLearningAnalytics> {
    await requirePlatformAdministrator(adminUserId)

    const mentorRows = await queryRows(
      `select count(*)::bigint as active_mentor_count
       from public.learning_mentors
       where status = 'active'`,
    ) as ActiveMentorCountRow[]

    const rows = await queryRows(
      `with platform_courses as (
         select
           course.id,
           course.slug,
           course.title,
       ...
       enrollment_progress as (
         ...
         inner join platform_courses course
    ...
    return {
      summary: {
        ...
        activeMentorCount: nonNegativeInteger(mentorRows[0]?.active_mentor_count),
        enrollmentCount,
...
export const adminLearningAnalyticsRepository = createAdminLearningAnalyticsRepository()
