...
const labels: Record<PublicProfile['profileType'], string> = {
  seafarer: 'Seafarer',
      ...
      {profile.skills.length ? (
        <div className="mt-4 flex flex-wrap gap-1.5">
          {profile.skills.slice(0, 3).map((skill) => (
            <span key={skill} className="rounded-full bg-mist-50 px-2.5 py-1 text-xs font-medium text-navy-900">
              {skill}
            </span>
          ))}
          {profile.skills.length > 3 ? (
            <span className="rounded-full bg-mist-50 px-2.5 py-1 text-xs font-medium text-muted">+{profile.skills.length - 3}</span>
          ) : null}
        </div>
      ) : null}
