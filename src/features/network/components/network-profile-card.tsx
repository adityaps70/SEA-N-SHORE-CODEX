  ...
  return (
    <Card className="flex h-full flex-col border border-mist-100 p-5">
      ...
      <div className="mt-4 space-y-2 text-sm text-muted">
        {profile.location ? (
          <p className="flex items-center gap-2"><MapPin aria-hidden="true" className="size-4 shrink-0" /><span className="truncate">{profile.location}</span></p>
        ) : null}
        {profile.rank || profile.currentCompany ? (
          <p className="flex items-center gap-2"><Ship aria-hidden="true" className="size-4 shrink-0" /><span className="truncate">{[profile.rank, profile.currentCompany].filter(Boolean).join(' · ')}</span></p>
        ) : null}
      </div>
      ...
      {profile.skills.length ? (
        <div className="mt-4 flex flex-wrap gap-1.5">
          {profile.skills.slice(0, 3).map((skill) => (
            <span key={skill} className="rounded-full bg-mist-50 px-2.5 py-1 text-xs font-medium text-navy-900">{skill}</span>
          ))}
      ...
      <div className="mt-auto pt-5">
        <RelationshipControls profileId={profile.id} initialRelationship={profile.relationship} />
