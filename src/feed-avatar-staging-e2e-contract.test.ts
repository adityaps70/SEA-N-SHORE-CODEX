...
describe('feed avatar staging E2E harness contract', () => {
  it('preserves the primary failure and uses cleanup-safe navigation', () => {
    ...
    expect(source).toContain('await addButton.click()')
    expect(source).toContain('await fileChooser.setFiles')
    expect(source).not.toContain('input.setInputFiles')
    ...
    expect(source).toContain("new URL(response.url()).pathname === '/profile'")
