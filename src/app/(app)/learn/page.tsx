...
function CourseCard({ course }: { course: MarketplaceCourse }) {
  ...
  return (
    <article className="group overflow-hidden rounded-[1.5rem] border border-mist-100 bg-white shadow-[var(--shadow-card)] transition duration-200 hover:-translate-y-0.5 hover:border-teal-200">
      <div className="relative min-h-40 overflow-hidden bg-gradient-to-br from-navy-950 via-navy-900 to-teal-800 p-5 text-white">
        ...
        <div className="relative flex h-full flex-col justify-between gap-8">
          ...
          <div>
            ...
            <p className="mt-1 text-sm font-medium text-white/75">Practical maritime learning from verified professionals</p>
          </div>
        </div>
      </div>
      ...
      <section className="mt-6 rounded-[1.5rem] border border-mist-100 bg-white p-4 shadow-[var(--shadow-card)] sm:p-5">
        <form action="/learn" method="get" className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <div className="relative flex-1">
            ...
            <input
              aria-label="Search maritime courses"
              type="search"
              name="search"
              defaultValue={search ?? ''}
              placeholder="Search courses, skills or mentors"
              className="w-full rounded-xl border border-mist-200 bg-mist-50/60 py-3 pl-10 pr-4 text-sm font-semibold text-navy-950 outline-none transition placeholder:font-normal placeholder:text-muted focus:border-teal-400 focus:bg-white"
            />
          </div>
          {category ? <input type="hidden" name="category" value={category} /> : null}
          <button
            type="submit"
            className="rounded-xl bg-navy-950 px-5 py-3 text-sm font-bold text-white transition hover:bg-navy-900"
          >
            Search courses
          </button>
          {hasFilters ? (
            <Link href="/learn" className="px-2 py-2 text-center text-sm font-bold text-teal-800 hover:text-teal-700">
              Clear filters
            </Link>
          ) : null}
        </form>
        ...
        <div className="mt-4 flex gap-2 overflow-x-auto pb-1" aria-label="Course categories">
          <Link
            href={allCoursesHref(search)}
            aria-current={category ? undefined : 'page'}
            className={`shrink-0 rounded-full border px-3 py-1.5 text-xs font-bold transition ${
              category ? 'border-mist-200 bg-white text-muted hover:border-teal-300 hover:text-teal-800' : 'border-navy-950 bg-navy-950 text-white'
            }`}
          >
            All courses
          </Link>
          {categories.map((item) => {
            ...
            return (
              <Link
                key={item}
                href={categoryHref(item, search)}
                aria-current={active ? 'page' : undefined}
                className={`shrink-0 rounded-full border px-3 py-1.5 text-xs font-bold transition ${
                  active ? 'border-navy-950 bg-navy-950 text-white' : 'border-mist-200 bg-white text-muted hover:border-teal-300 hover:text-teal-800'
                }`}
              >
                {item}
              </Link>
      ...
      <section className="mt-7">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <h2 className="mt-1 text-2xl font-bold tracking-tight text-navy-950">
              {hasFilters ? 'Courses matching your filters' : 'Published maritime courses'}
            </h2>
          </div>
        ...
        {courses.length ? (
          ...
          <div className="mt-4 rounded-[1.5rem] border border-dashed border-mist-200 bg-white p-8 text-center shadow-[var(--shadow-card)] sm:p-10">
            ...
            <h2 className="mt-4 text-xl font-bold text-navy-950">No published courses match these filters yet.</h2>
            <p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-muted">
              Try another maritime category or remove the current filters. Only courses that have completed Sea N Shore review appear here.
            </p>
            <Link
              href="/learn"
