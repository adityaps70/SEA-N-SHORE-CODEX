...
case "$PHASE" in
  ...
  verify-republish)
    ...
    ROW=$(jq -r '.records[0] | map(.stringValue // "") | @tsv' <<<"$RESULT")
    APPLICATION_ROW_ID=$(cut -f1 <<<"$ROW")
    APPLICATION_STATUS=$(cut -f2 <<<"$ROW")
    APPLICANT_MATCH=$(cut -f3 <<<"$ROW")
    EVENT_COUNT=$(cut -f4 <<<"$ROW")
    [[ "$APPLICATION_ROW_ID" =~ ^[0-9a-f-]{36}$ && "$APPLICATION_STATUS" == applied && "$APPLICANT_MATCH" == true && "$EVENT_COUNT" == 1 ]] || { echo "Job application state mismatch: $ROW" >&2; exit 1; }
    echo 'ORGANIZATION_HIRING_E2E_JOB_APPLICATION_VERIFIED=true'
    ;;
...
published\ttrue\ttrue' ]] || { echo "Republished job state mismatch: $ROW" >&2; exit 1; }
