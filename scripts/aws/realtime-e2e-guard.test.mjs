...
test('realtime messaging failure captures outbox worker and fanout diagnostics before browser cleanup', () => {
  ...
  assert.match(browser, /REALTIME_E2E_BROWSER_UNREAD_COUNT=/)
  assert.match(browser, /REALTIME_E2E_BROWSER_UNREAD_BADGES=/)
  assert.match(browser, /REALTIME_E2E_BROWSER_VISIBILITY=/)
  assert.match(remote, /^  diagnose-message\)/m)
  assert.match(remote, /REALTIME_MESSAGE_DIAG_RECIPIENT_LAST_READ_MESSAGE_ID=/)
  assert.match(remote, /REALTIME_MESSAGE_DIAG_RECIPIENT_UNREAD_COUNT=/)
  assert.match(remote, /REALTIME_MESSAGE_DIAG_LATEST_MESSAGE_ID=/)
  assert.match(remote, /REALTIME_MESSAGE_DIAG_LATEST_MESSAGE_AT=/)
  assert.match(remote, /REALTIME_MESSAGE_DIAG_READ_CURSOR_MATCH_LATEST=/)
  assert.match(remote, /REALTIME_MESSAGE_DIAG_LAST_READ_EVENT_MESSAGE_ID=/)
...
test('realtime messaging E2E covers rich composer, arbitrary reactions, replies, attachments and unsend', () => {
  ...
  assert.match(browser, /Edit message/)
  assert.match(browser, /Save edit/)
  ...
  assert.match(browser, /REALTIME_E2E_EDIT_VERIFIED=true/)
  ...
  assert.match(remote, /edited_at is not null/i)
