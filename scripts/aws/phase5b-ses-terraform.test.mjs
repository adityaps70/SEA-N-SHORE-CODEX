...
test('Phase 5B SES identity tags reconcile through a guarded exact-head SSM path', () => {
  const runnerUrl = new URL('scripts/aws/ses-identity-tags.sh', repoRoot)
  const actionUrl = new URL('scripts/aws/ses-identity-tags-action.txt', repoRoot)
  const workflowUrl = new URL('.github/workflows/aws-ses-identity-tags.yml', repoRoot)
  ...
  assert.match(runner, /aws sesv2 list-tags-for-resource/)
  assert.match(runner, /aws sesv2 get-email-identity/)
  assert.match(runner, /VerifiedForSendingStatus/)
  assert.match(runner, /VerificationStatus/)
  assert.match(runner, /DkimAttributes\.Status/)
  assert.match(runner, /CurrentSigningKeyLength/)
  assert.match(runner, /NextSigningKeyLength/)
  assert.match(runner, /Environment[^\n]+staging/)
  assert.match(runner, /ManagedBy[^\n]+Terraform/)
  assert.match(runner, /Project[^\n]+Sea N Shore/)
  assert.match(runner, /SES_IDENTITY_TAGS_LIVE_BEFORE=/)
  assert.match(runner, /SES_IDENTITY_TAGS_DESIRED=/)
  assert.match(runner, /SES_IDENTITY_TAGS_PLAN_ONLY_NO_MUTATION/)
  assert.match(runner, /\[\[ "\$LIVE_TAGS_BEFORE" == "\[\]" \]\]/)
  assert.match(runner, /git ls-remote origin refs\/heads\/feat\/aws-native-phase-0-1/)
  assert.match(runner, /aws sesv2 tag-resource/)
  assert.match(runner, /SES_IDENTITY_TAGS_APPLY_VERIFIED=true/)
  assert.match(runner, /SES_IDENTITY_TAGS_LIVE_SHAPE_UNCHANGED=true/)
  assert.doesNotMatch(runner, /aws\s+sesv2\s+untag-resource/)
  assert.doesNotMatch(runner, /aws\s+sesv2\s+(?:create|update|put|delete)-/)
  assert.doesNotMatch(runner, /aws\s+cognito-idp\s+update-/)
  assert.doesNotMatch(runner, /terraform[^\n]+(?:import|apply)/)

  const workflow = fs.readFileSync(workflowUrl, 'utf8')
  assert.match(workflow, /name: AWS SES Identity Tags/)
  assert.match(workflow, /branches:\s*\n\s*- feat\/aws-native-phase-0-1/)
  assert.match(workflow, /Wait for exact-head AWS Infrastructure CI/)
  assert.match(workflow, /role-to-assume: \$\{\{ vars\.AWS_ROLE_TO_ASSUME \}\}/)
  assert.match(workflow, /aws ssm send-command/)
  assert.match(workflow, /SES_IDENTITY_TAGS_EXPECTED_SHA/)
  assert.match(workflow, /bash scripts\/aws\/ses-identity-tags\.sh/)
  assert.doesNotMatch(workflow, /terraform\s+-chdir=/)
})