...
test('Phase 5B has a read-only SSM audit for SES Terraform state versus live AWS', () => {
  ...
  assert.match(script, /aws_sesv2_configuration_set\.transactional/)
  assert.match(script, /aws_sesv2_email_identity\.transactional_domain/)
  assert.match(script, /aws_ses_identity_policy\.cognito_sender/)
