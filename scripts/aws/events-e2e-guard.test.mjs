...
test('events e2e proves authenticated create, RSVP, edit, withdrawal, cancellation and durable Aurora state', () => {
  ...
  assert.match(browser, /Register for event/)
  assert.doesNotMatch(browser, /Attend event/)
  assert.match(browser, /Withdraw attendance/)
  assert.match(browser, /EVENTS_E2E_CARD_NAV_VERIFIED=true/)
  assert.match(browser, /You're attending/)
  assert.match(browser, /Google Calendar/)
  assert.match(browser, /Download \.ics/)
  assert.match(browser, /setEventDateTime/)
  assert.match(browser, /Next month/)
