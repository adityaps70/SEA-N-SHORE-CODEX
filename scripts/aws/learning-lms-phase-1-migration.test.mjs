...
assert.match(workflow, /mktemp -d \/var\/tmp\/sea-n-shore-learning-lms-phase-1\.XXXXXXXX/)
assert.match(workflow, /git clone --quiet --single-branch --branch feat\/aws-native-phase-0-1/)
assert.match(workflow, /Wait and surface migration evidence/)
assert.match(workflow, /get-command-invocation/)
assert.doesNotMatch(workflow, /aws ssm wait command-executed/)
assert.doesNotMatch(workflow, /cd \/home\/ssm-user\/sea-n-shore-staging/)
