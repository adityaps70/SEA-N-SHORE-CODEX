...
POLICY_NAME="sea-n-shore-staging-ecs-task-media"
IMPORT_ID="${ROLE_NAME}:${POLICY_NAME}"
MEDIA_OBJECT_ARN="arn:aws:s3:::sea-n-shore-staging-310356785722-media/*"
ACTION_FILE="scripts/aws/ecs-task-media-policy-state-action.txt"

[[ "${ECS_TASK_MEDIA_POLICY_STATE_EXPECTED_SHA:-}" =~ ^[0-9a-f]{40}$ ]] || {
  echo "ECS_TASK_MEDIA_POLICY_STATE_EXPECTED_SHA must be an exact commit SHA." >&2
  exit 1
}
[[ "$(git rev-parse HEAD)" == "$ECS_TASK_MEDIA_POLICY_STATE_EXPECTED_SHA" ]]
[[ "$(git remote get-url origin)" == "https://github.com/adityaps70/SEA-N-SHORE-CODEX.git" ]]
git diff --quiet HEAD -- \
  infra/aws/app/storage.tf \
  scripts/aws/ecs-task-media-policy-state.sh \
  scripts/aws/ecs-task-media-policy-state-action.txt \
  scripts/aws/ecs-task-media-policy-state-release.test.mjs \
  .github/workflows/aws-ecs-task-media-policy-state.yml
[[ "$(aws sts get-caller-identity --query Account --output text)" == "$EXPECTED_ACCOUNT" ]]

ACTION="$(tr -d '[:space:]' < "$ACTION_FILE")"
case "$ACTION" in plan|apply-once) ;; *) echo "Unsupported ECS task media policy state action." >&2; exit 1 ;; esac

WORK_DIR="$(mktemp -d "$PWD/.ecs-task-media-policy-state.XXXXXXXX")"
...
STATE_COUNT="$(jq '[.resources[] | select(.mode=="managed" and .type=="aws_iam_role_policy" and .name=="ecs_task_media")] | length' "$WORK_DIR/state.json")"
if [[ "$STATE_COUNT" == "1" ]]; then
  echo "ECS_TASK_MEDIA_POLICY_STATE_ALREADY_RECONCILED=true"
  exit 0
fi
[[ "$STATE_COUNT" == "0" ]] || {
  echo "Unexpected ECS task media policy state count: $STATE_COUNT" >&2
  exit 1
}
...
verify_live_policy "$WORK_DIR/live-policy-before.json"
echo "ECS_TASK_MEDIA_POLICY_STATE_LIVE_POLICY_MATCHES_DESIRED=true"
...
PY
echo "ECS_TASK_MEDIA_POLICY_STATE_PROVIDER_LOCK_VERIFIED=true"
...
' "$WORK_DIR/plan-before.json" >/dev/null || {
  echo "ECS task media policy pre-import plan contains unexpected actual changes." >&2
...
echo "ECS_TASK_MEDIA_POLICY_STATE_ACTION=$ACTION"
echo "ECS_TASK_MEDIA_POLICY_STATE_COUNT_BEFORE=0"
echo "STATE_SERIAL_BEFORE=$STATE_SERIAL_BEFORE"
echo "ECS_TASK_MEDIA_POLICY_STATE_PLAN_VERIFIED=IMPORT_ONLY"
echo "PLAN_SHA256=$(sha256sum "$WORK_DIR/before.tfplan" | cut -d' ' -f1)"

if [[ "$ACTION" == "plan" ]]; then
  echo "ECS_TASK_MEDIA_POLICY_STATE_PLAN_ONLY_NO_IMPORT"
  exit 0
fi

[[ "$(git ls-remote origin refs/heads/feat/aws-native-phase-0-1 | cut -f1)" == "$ECS_TASK_MEDIA_POLICY_STATE_EXPECTED_SHA" ]]
...
verify_live_policy "$WORK_DIR/live-policy-pre-import.json"
echo "IMPORTING_EXISTING_ECS_TASK_MEDIA_POLICY_INTO_TERRAFORM_STATE"
...
[[ "$ACTUAL_AFTER" == "0" ]] || {
  echo "Imported ECS task media policy still has Terraform drift; refusing success." >&2
...
STATE_COUNT_AFTER="$(jq '[.resources[] | select(.mode=="managed" and .type=="aws_iam_role_policy" and .name=="ecs_task_media")] | length' "$WORK_DIR/state-after.json")"
[[ "$STATE_COUNT_AFTER" == "1" ]]
...
echo "ECS_TASK_MEDIA_POLICY_STATE_COUNT_AFTER=1"
echo "ECS_TASK_MEDIA_POLICY_STATE_LIVE_POLICY_MATCHES_DESIRED_AFTER=true"
echo "ECS_TASK_MEDIA_POLICY_STATE_IMPORT_VERIFIED=true"
