...
case "$PHASE" in
  ...
  verify-durable)
    ...
    FOLLOWUP_COUNT=$(cut -f4 <<<"$ROW")
    INJECTED_COUNT=$(cut -f5 <<<"$ROW")
    ...
    [[ "$DELETED_COUNT" -ge 1 ]]
    [[ "$REACTION_COUNT" == 1 ]]
    [[ "$UPDATED_EVENT_COUNT" -ge 2 ]]
    [[ "$LIVE_FILE_COUNT" == 0 ]]
    PHOTO_STORAGE_PATH=$(sql "SELECT attachment_storage_path FROM public.messages WHERE conversation_id='$CONVERSATION_ID'::uuid AND attachment_name='$RICH_PHOTO_NAME' AND deleted_at IS NULL LIMIT 1" | jq -r '.records[0][0].stringValue // empty')
    [[ "$PHOTO_STORAGE_PATH" == messages/*/"$CONVERSATION_ID"/*.png ]]
    MEDIA_BUCKET='sea-n-shore-staging-310356785722-media'
    echo "REALTIME_E2E_RICH_PHOTO_STORAGE_PATH=$PHOTO_STORAGE_PATH"
    echo "REALTIME_E2E_RICH_MEDIA_BUCKET=$MEDIA_BUCKET"
    echo 'REALTIME_E2E_DURABLE_RICH_MESSAGING_VERIFIED=true'
  ...
  cleanup)
    ...
    if [[ "$CONVERSATION_ID" =~ ^[0-9a-f-]{36}$ ]]; then
      if [[ "$HAS_MESSAGES" == true ]]; then
        MEDIA_BUCKET='sea-n-shore-staging-310356785722-media'
        ATTACHMENT_KEYS=$(sql "SELECT attachment_storage_path FROM public.messages WHERE conversation_id='$CONVERSATION_ID'::uuid AND attachment_storage_path IS NOT NULL" | jq -r '.records[]?[0].stringValue // empty')
        if [[ -n "$ATTACHMENT_KEYS" ]]; then
          while IFS= read -r KEY; do
            [[ -n "$KEY" ]] || continue
            set +e
            aws s3api delete-object --region "$AWS_REGION" --bucket "$MEDIA_BUCKET" --key "$KEY" >/dev/null 2>&1
            set -e
          done <<<"$ATTACHMENT_KEYS"
        fi
      fi
