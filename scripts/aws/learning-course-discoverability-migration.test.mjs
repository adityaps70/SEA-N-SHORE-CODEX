...
assert.match(sql, /create or replace function public\.sync_learning_course_discoverability\(\)/i)
assert.match(sql, /new\.status = 'published'[\s\S]*new\.is_discoverable := true/i)
assert.match(sql, /new\.status is distinct from 'published'[\s\S]*new\.is_discoverable := false/i)
assert.match(sql, /create or replace trigger learning_course_discoverability_sync/i)
assert.match(sql, /before update of status on public\.learning_courses/i)
assert.match(sql, /execute function public\.sync_learning_course_discoverability\(\)/i)
...
assert.match(script, /expected exactly five additive discoverability statements/)
assert.match(script, /create\\s\+or\\s\+replace\\s\+function/)
assert.match(script, /create\\s\+or\\s\+replace\\s\+trigger/)
assert.match(script, /column_default IN \('false','false::boolean'\)/)
assert.match(script, /sync_learning_course_discoverability/)
assert.match(script, /learning_course_discoverability_sync/)
