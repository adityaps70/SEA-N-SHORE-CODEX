...
add constraint profiles_slug_length_check
...
create table public.maritime_profiles (
  ...
  constraint maritime_profiles_rank_check
    check (rank is null or (rank = btrim(rank) and char_length(rank) between 1 and 100)),
  constraint maritime_profiles_current_company_check
    check (current_company is null or (current_company = btrim(current_company) and char_length(current_company) between 1 and 160)),
  constraint maritime_profiles_current_vessel_check
    check (current_vessel is null or (current_vessel = btrim(current_vessel) and char_length(current_vessel) between 1 and 160)),
  constraint maritime_profiles_experience_check
    check (sailing_experience_years is null or sailing_experience_years between 0 and 70),
  constraint maritime_profiles_vessel_types_check
    check (cardinality(vessel_types) <= 20),
  constraint maritime_profiles_trading_areas_check
    check (cardinality(trading_areas) <= 20),
  constraint maritime_profiles_availability_check
    check (availability is null or (availability = btrim(availability) and char_length(availability) between 1 and 100))
);
...
create table public.profile_skills (
  ...
  constraint profile_skills_skill_check
    check (skill = btrim(skill) and char_length(skill) between 1 and 80)
);
...
create trigger normalize_maritime_profile
before insert or update on public.maritime_profiles
for each row execute procedure private.normalize_maritime_profile();
...
as $$
begin
  new.skill := btrim(new.skill);
  return new;
end;
$$;

create trigger normalize_profile_skill
before insert or update on public.profile_skills
for each row execute procedure private.normalize_profile_skill();

revoke all on function private.normalize_profile_skill() from public, anon, authenticated;

alter table public.maritime_profiles enable row level security;
alter table public.profile_skills enable row level security;

create policy "public maritime details follow profile visibility"
on public.maritime_profiles for select to anon
using (exists (
  select 1
  from public.profiles
  where id = maritime_profiles.user_id
    and account_status = 'active'
    and onboarding_completed_at is not null
));

create policy "members read visible maritime details"
on public.maritime_profiles for select to authenticated
using (
...
create policy "members insert their maritime details"
on public.maritime_profiles for insert to authenticated
with check (
  user_id = (select auth.uid())
  and exists (
    select 1
    from public.profiles
    where id = maritime_profiles.user_id
      and profile_type in ('seafarer', 'maritime_professional')
  )
);

create policy "members update their maritime details"
on public.maritime_profiles for update to authenticated
using (
  user_id = (select auth.uid())
  and exists (
    select 1
...
create policy "members delete their maritime details"
on public.maritime_profiles for delete to authenticated
using (
  user_id = (select auth.uid())
  and exists (
    select 1
...
create policy "public skills follow profile visibility"
on public.profile_skills for select to anon
using (exists (
  select 1
  from public.profiles
  where id = profile_skills.user_id
    and account_status = 'active'
    and onboarding_completed_at is not null
));

create policy "members read visible skills"
...
create policy "members insert their skills"
on public.profile_skills for insert to authenticated
with check (user_id = (select auth.uid()));

create policy "members delete their skills"
on public.profile_skills for delete to authenticated
using (user_id = (select auth.uid()));

revoke all on table public.maritime_profiles, public.profile_skills from public, anon, authenticated;
grant select on table public.maritime_profiles, public.profile_skills to anon, authenticated;
grant insert, delete on table public.maritime_profiles, public.profile_skills to authenticated;
grant update (
  rank, current_company, current_vessel, sailing_experience_years,
  vessel_types, trading_areas, shore_career_preference, availability
) on public.maritime_profiles to authenticated;
...
create or replace function public.complete_onboarding(
  ...
  p_current_vessel text,
  p_sailing_experience_years numeric,
  p_vessel_types text[],
  p_trading_areas text[],
  p_shore_career_preference boolean,
  p_availability text
)
returns void
language plpgsql
