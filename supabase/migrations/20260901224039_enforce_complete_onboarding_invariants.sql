...
begin
  ...
  then
    raise check_violation using message = 'completed maritime profiles require maritime details';
  end if;
...
create or replace function private.prevent_required_maritime_delete()
returns trigger
language plpgsql
security invoker
set search_path = ''
...
create trigger prevent_required_maritime_delete
before delete on public.maritime_profiles
for each row execute procedure private.prevent_required_maritime_delete();

revoke all on function private.prevent_required_maritime_delete()
from public, anon, authenticated;

drop policy "public maritime details follow profile visibility"
...
drop policy "members read visible maritime details"
on public.maritime_profiles;
